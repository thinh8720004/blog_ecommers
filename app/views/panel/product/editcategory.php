...
<div class="container-form-update-article">
    ...
    foreach ($categorybyid as $key => $cate) {
        ?>
        <form action="<?php echo BASE_URL ?>/product/updateCategoryProduct/<?php echo $cate['id_category_product'] ?>"
            method="post">
            <div class="form-group">
                <label for="nameCategory">Name Category Product</label>
                <input type="text" value="<?php echo $cate['title_category_product'] ?>" name="title_category_product"
                    class=" form-control">
            </div>
            <div class="form-group">
                <label for="describeArticle">Describe Category Product</label>
                <textarea id="editor1" type="text" value="<?php echo $cate['describe_category_product'] ?>"
                    name="desc_category_product" class=" form-control"></textarea>
            </div>
